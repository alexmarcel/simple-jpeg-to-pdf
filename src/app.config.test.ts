...
describe('deployment configuration', () => {
  ...
  it('contains valid application defaults and limits', () => {
    ...
    expect(appConfig.limits.importWarningBytes).toBe(100 * 1024 * 1024)
    expect(appConfig.limits.importWarningPages).toBe(500)
    expect(appConfig.limits.maxFileBytes).toBe(500 * 1024 * 1024)
    expect(appConfig.limits.maxPdfPages).toBe(2_000)
    expect(appConfig.limits.maxImagePixels).toBe(100_000_000)
    expect(appConfig.limits.maxProjectBytes).toBe(1024 * 1024 * 1024)
    expect(appConfig.limits.recoveryMaxAgeMs).toBe(24 * 60 * 60 * 1000)
  })
})
